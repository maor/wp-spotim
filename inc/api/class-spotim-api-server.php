...
class SpotIM_API_Server {
	...
	public function __construct( $path ) {
		...
		$handler_class = apply_filters( 'spotim_api_default_response_handler', 'SpotIM_API_JSON_Handler', $this->path, $this );
	...
	public function check_authentication() {
		...
		$user = apply_filters( 'spotim_api_check_authentication', null, $this );
		...
		/*elseif ( ! is_wp_error( $user ) )
			$user = new WP_Error( 'spotim_api_authentication_error', __( 'Invalid authentication method', 'wp-spotim' ), array( 'code' => 500 ) );*/
	 ...
	 *
	 * @since 2.1
	 * @uses SpotIM_API_Server::dispatch()
	 */
	public function serve_request() {

		do_action( 'spotim_api_server_before_serve', $this );

		$this->header( 'Content-Type', $this->handler->get_content_type(), true );

		// the API is enabled by default
		if ( ! apply_filters( 'spotim_api_enabled', true, $this ) || ( 'no' === get_option( 'spotim_api_enabled' ) ) ) {

			$this->send_status( 404 );

			echo $this->handler->generate_response( array( 'errors' => array( 'code' => 'spotim_api_disabled', 'message' => 'The SpotIM API is disabled on this site' ) ) );
		...
		$served = apply_filters( 'spotim_api_serve_request', false, $result, $this );
	...
	public function get_routes() {
		...
		$endpoints = apply_filters( 'spotim_api_endpoints', $endpoints );
	...
	public function dispatch() {
		...
		switch ( $this->method ) {
			...
			default:
				return new WP_Error( 'spotim_api_unsupported_method', __( 'Unsupported request method', 'wp-spotim' ), array( 'status' => 400 ) );
		}
		...
		foreach ( $this->get_routes() as $route => $handlers ) {
			foreach ( $handlers as $handler ) {
				...
				if ( ! is_callable( $callback ) )
					return new WP_Error( 'spotim_api_invalid_handler', __( 'The handler for the route is invalid', 'wp-spotim' ), array( 'status' => 500 ) );
				...
				$args = apply_filters( 'spotim_api_dispatch_args', $args, $callback );
		...
		return new WP_Error( 'spotim_api_no_route', __( 'No route was found matching the URL and request method', 'wp-spotim' ), array( 'status' => 404 ) );
	}
	...
	protected function sort_callback_params( $callback, $provided ) {
		...
		foreach ( $wanted as $param ) {
			...
			elseif ( $param->isDefaultValueAvailable() ) {
				...
				return new WP_Error( 'spotim_api_missing_callback_param', sprintf( __( 'Missing parameter %s', 'wp-spotim' ), $param->getName() ), array( 'status' => 400 ) );
			}
		}
	...
	public function add_pagination_headers( $query ) {
		...
		do_action( 'spotim_api_pagination_headers', $this, $query );
	}
	...
	public function format_datetime( $timestamp, $convert_to_utc = false ) {
		...
		if ( $convert_to_utc ) {
			$timezone = new DateTimeZone( spotim_timezone_string() );
		} else {
