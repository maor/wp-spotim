<?php
/**
 * SpotIM API Authentication Class
 *
 * @author      Spot.IM
 * @category    API
 * @package     SpotIM/API
...
class SpotIM_API_Authentication {

	/**
	 * Setup class
	 *
	 * @since 2.1
	 * @return SpotIM_API_Authentication
	 */
	public function __construct() {

		// to disable authentication, hook into this filter at a later priority and return a valid WP_User
		add_filter( 'SpotIM_api_check_authentication', array( $this, 'authenticate' ), 0 );
	}
	...
	public function authenticate( $user ) {
		...
		if ( '/' === spotim_instance()->spotim_api->server->path ) {
		...
		try {
			...
			if ( is_ssl() ) {
				...
				$user = $this->perform_token_authentication();
			}
			...
			$user = new WP_Error( 'SpotIM_api_authentication_error', $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	...
	private function perform_ssl_authentication() {
		...
		$params = spotim_instance()->spotim_api->server->params['GET'];
		...
		if ( ! empty( $_SERVER['PHP_AUTH_USER'] ) ) {
			...
			throw new Exception( __( 'Consumer Key is missing', 'wp-spotim' ), 404 );
		}
		...
		if ( ! empty( $_SERVER['PHP_AUTH_PW'] ) ) {
			...
			throw new Exception( __( 'Consumer Secret is missing', 'wp-spotim' ), 404 );
		}

		$user = $this->get_user_by_consumer_key( $consumer_key );

		if ( ! $this->is_consumer_secret_valid( $user, $consumer_secret ) ) {
			throw new Exception( __( 'Consumer Secret is invalid', 'wp-spotim' ), 401 );
		}
	...
	private function perform_token_authentication() {

		$params = spotim_instance()->spotim_api->server->params['GET'];

		// check if token equals one in DB
		if ( ! isset( $params['spot_token'] ) ) {
			throw new Exception( 'spot_token not provided.', 404 );
			return false;
		}

		$auth = get_option( SPOTIM_AUTH_OPTION );

		if ( isset( $auth['spot_token'] ) && $params['spot_token'] == $auth['spot_token'] )
			return true;
		else
			throw new Exception( 'Auth token is not valid', 401 );
	...
	private function perform_oauth_authentication() {
		...
		foreach ( $param_names as $param_name ) {
			...
			if ( empty( $params[ $param_name ] ) )
				throw new Exception( sprintf( __( '%s parameter is missing', 'wp-spotim' ), $param_name ), 404 );
		}
	...
	private function get_user_by_consumer_key( $consumer_key ) {
		...
		$user_query = new WP_User_Query(
			array(
				'meta_key'   => 'SpotIM_api_consumer_key',
		...
		if ( empty( $users[0] ) )
			throw new Exception( __( 'Consumer Key is invalid', 'wp-spotim' ), 401 );
	...
	private function is_consumer_secret_valid( WP_User $user, $consumer_secret ) {
		...
		return hash_equals( $user->SpotIM_api_consumer_secret, $consumer_secret );
	}
	...
	private function check_oauth_timestamp_and_nonce( $user, $timestamp, $nonce ) {
		...
		if ( ( $timestamp < time() - $valid_window ) ||  ( $timestamp > time() + $valid_window ) )
			throw new Exception( __( 'Invalid timestamp', 'wp-spotim' ) );

		$used_nonces = $user->SpotIM_api_nonces;

		if ( empty( $used_nonces ) )
			$used_nonces = array();

		if ( in_array( $nonce, $used_nonces ) )
			throw new Exception( __( 'Invalid nonce - nonce has already been used', 'wp-spotim' ), 401 );
		...
		update_user_meta( $user->ID, 'SpotIM_api_nonces', $used_nonces );
	}
	...
	public function check_api_key_permissions( $user ) {
		...
		$key_permissions = 'read_write'/*$user->SpotIM_api_key_permissions*/;
