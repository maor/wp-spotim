...
class SpotIM_API_Resource {

	/** @var SpotIM_API_Server the API server */
	protected $server;
	 ...
	 *
	 * @since 2.1
	 * @param SpotIM_API_Server $server
	 * @return SpotIM_API_Resource
	 */
	public function __construct( SpotIM_API_Server $server ) {

		$this->server = $server;

		// automatically register routes for sub-classes
		add_filter( 'spotim_api_endpoints', array( $this, 'register_routes' ) );

		// maybe add meta to top-level resource responses
		foreach ( array( 'order', 'coupon', 'customer', 'product', 'report' ) as $resource ) {
			add_filter( "spotim_api_{$resource}_response", array( $this, 'maybe_add_meta' ), 15, 2 );
		}
		...
		foreach ( $response_names as $name ) {
			 ...
			 */
			add_filter( "spotim_api_{$name}_response", array( $this, 'filter_response_fields' ), 20, 3 );
		}
	}
	...
	protected function validate_request( $id, $type, $context ) {
		...
		// Validate ID
		if ( empty( $id ) ) {
			return new WP_Error( "spotim_api_invalid_{$resource_name}_id", sprintf( __( 'Invalid %s ID', 'wp-spotim' ), $type ), array( 'status' => 404 ) );
		}
		...
		if ( 'customer' !== $type ) {
			...
			if ( null === $post ) {
				return new WP_Error( "spotim_api_no_{$resource_name}_found", sprintf( __( 'No %s found with the ID equal to %s', 'wp-spotim' ), $resource_name, $id ), array( 'status' => 404 ) );
			}
			...
			// Validate post type
			if ( $type !== $post_type ) {
				return new WP_Error( "spotim_api_invalid_{$resource_name}", sprintf( __( 'Invalid %s', 'wp-spotim' ), $resource_name ), array( 'status' => 404 ) );
			}
			...
			switch ( $context ) {
				...
				case 'read':
					if ( ! $this->is_readable( $post ) )
						return new WP_Error( "spotim_api_user_cannot_read_{$resource_name}", sprintf( __( 'You do not have permission to read this %s', 'wp-spotim' ), $resource_name ), array( 'status' => 401 ) );
					break;

				case 'edit':
					if ( ! $this->is_editable( $post ) )
						return new WP_Error( "spotim_api_user_cannot_edit_{$resource_name}", sprintf( __( 'You do not have permission to edit this %s', 'wp-spotim' ), $resource_name ), array( 'status' => 401 ) );
					break;

				case 'delete':
					if ( ! $this->is_deletable( $post ) )
						return new WP_Error( "spotim_api_user_cannot_delete_{$resource_name}", sprintf( __( 'You do not have permission to delete this %s', 'wp-spotim' ), $resource_name ), array( 'status' => 401 ) );
					break;
			}
		}
	...
	protected function merge_query_args( $base_args, $request_args ) {
		...
		$args = apply_filters( 'spotim_api_query_args', $args, $request_args );
	 ...
	 * @param object $resource the resource object (e.g SpotIM_Order)
	 * @return mixed
	 */
	public function maybe_add_meta( $data, $resource ) {
		...
		if ( isset( $this->server->params['GET']['filter']['meta'] ) && 'true' === $this->server->params['GET']['filter']['meta'] && is_object( $resource ) ) {
			...
			} elseif ( is_a( $resource, 'SpotIM_Product_Variation' ) ) {
	 ...
	 * @param object $resource the object that provided the response data, e.g. SpotIM_Coupon or SpotIM_Order
	...
	protected function delete( $id, $type, $force = false ) {
		...
		if ( 'customer' === $type ) {
			...
			if ( $result )
				return array( 'message' => __( 'Permanently deleted customer', 'wp-spotim' ) );
			else
				return new WP_Error( 'spotim_api_cannot_delete_customer', __( 'The customer cannot be deleted', 'wp-spotim' ), array( 'status' => 500 ) );
			...
			if ( ! $result )
				return new WP_Error( "spotim_api_cannot_delete_{$resource_name}", sprintf( __( 'This %s cannot be deleted', 'wp-spotim' ), $resource_name ), array( 'status' => 500 ) );

			if ( $force ) {
				return array( 'message' => sprintf( __( 'Permanently deleted %s', 'wp-spotim' ), $resource_name ) );

			} else {

				$this->server->send_status( '202' );

				return array( 'message' => sprintf( __( 'Deleted %s', 'wp-spotim' ), $resource_name ) );
			}
		}
	}
	 ...
	 *
	 * @since 2.1
	 * @see SpotIM_API_Resource::check_permission()
